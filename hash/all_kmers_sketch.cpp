...
void process_fasta_file(const std::filesystem::path& filepath, int k, CountMinSketch& cms) {
    ...
    while (std::getline(file, line)) {
        ...
        if (line[0] == '>') {
            ...
            if (!sequence.empty()) {
                process_sequence(sequence, k, cms); 
            }
            seq_buffer.str(""); 
        } else {
...
int main() {
    ...
    std::string SKETCH_FILE = "./all_kmers_sketchT.bin"; 
    std::string METRICS_FILE = "./all_kmers_metricsT.txt"; 
    int K_MER = 21; 
    
    int CMS_WIDTH = 2000000;
    int CMS_DEPTH = 5;

    std::cout << "Iniciando Pipeline C++ (TODOS los K-mers)..." << std::endl;
    ...
    std::cout << "Total de inserciones en el Sketch (N): " << total_insertions << std::endl;
    ...
    if (metrics_file.is_open()) {
        metrics_file << "--- Métricas de Procesamiento (Todos los K-mers) ---" << std::endl;
