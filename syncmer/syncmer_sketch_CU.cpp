...
void process_fasta_file(const std::filesystem::path& filepath, int k, int s, int t, CountMinSketch& cms) {
    ...
    while (std::getline(file, line)) {
        ...
        if (line[0] == '>') {
            ...
            if (!sequence.empty()) process_sequence_syncmer(sequence, k, s, t, cms);
            seq_buffer.str(""); 
        } else {
...
int main() {
    ...
    std::string SKETCH_FILE = "./syncmer_sketch_CU.bin";
    std::string METRICS_FILE = "./syncmer_metrics_CU.txt";
    int K_MER = 21; 
    int S_MER = 11; 
    int T_OFFSET = 5; 
    ...
    std::cout << "Iniciando Pipeline C++ (Open Syncmers - CONSERVATIVE UPDATE)..." << std::endl;
    ...
    if (metrics_file.is_open()) {
        metrics_file << "--- Métricas Conservative Update (Syncmers) ---" << std::endl;
