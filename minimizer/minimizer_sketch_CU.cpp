...
void process_sequence(const std::string& sequence, int w, int k, CountMinSketch& cms) {
    if (sequence.length() < k + w - 1) return;
    ...
    for (int i = 0; i <= (int)sequence.length() - k; ++i) {
        ...
        if (!is_valid_kmer(kmer)) {
            mq.clear(); kmer_buffer.clear(); continue; 
        }
        ...
        if (i >= w - 1) {
            ...
            if (!kmer_buffer.empty()) kmer_buffer.pop_front();
        }
    }
}
...
void process_fasta_file(const std::filesystem::path& filepath, int w, int k, CountMinSketch& cms) {
    ...
    while (std::getline(file, line)) {
        ...
        if (line[0] == '>') {
            ...
            if (!sequence.empty()) process_sequence(sequence, w, k, cms);
            seq_buffer.str(""); 
        } else {
...
int main() {
    ...
    std::string SKETCH_FILE = "./minimizer_sketch_CU.bin";
    std::string METRICS_FILE = "./processing_metrics_CU.txt";
    int W_WINDOW = 10;
    int K_MER = 21;
    int CMS_WIDTH = 2000000;
    int CMS_DEPTH = 5;

    std::cout << "Iniciando Pipeline C++ (Minimizers - CONSERVATIVE UPDATE)..." << std::endl;
    ...
    std::cout << "Total Inserciones: " << total_insertions << std::endl;
    ...
    if (metrics_file.is_open()) {
        metrics_file << "--- Métricas Conservative Update (Minimizers) ---" << std::endl;
